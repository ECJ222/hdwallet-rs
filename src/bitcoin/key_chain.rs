use crate::{error::Error, BitcoinExPrivateKey, ChainPath, ChainPathError, KeyIndex, SubPath};
...
pub struct Derivation {
    ...
    pub depth: u8,
    /// parent key
    pub parent_key: Option<BitcoinExPrivateKey>,
...
/// use hdwallet::{KeyChain, DefaultKeyChain, ChainPath, BitcoinExPrivateKey};
/// use rand;
///
/// let mut rng = rand::thread_rng();
/// let master_key = BitcoinExPrivateKey::random(&mut rng).unwrap();
...
pub trait KeyChain {
    ...
    ) -> Result<(BitcoinExPrivateKey, Derivation), Error>;
}

pub struct DefaultKeyChain {
    master_key: BitcoinExPrivateKey,
}

impl DefaultKeyChain {
    #[allow(dead_code)]
    pub fn new(master_key: BitcoinExPrivateKey) -> Self {
...
mod tests {
    use super::*;
    use crate::{traits::Serialize, BitcoinExPublicKey};
    use base58::ToBase58;
    use ring::digest;
    ...
    pub enum ExtendedKey {
        PrivKey(BitcoinExPrivateKey),
        PubKey(BitcoinExPublicKey),
    }
